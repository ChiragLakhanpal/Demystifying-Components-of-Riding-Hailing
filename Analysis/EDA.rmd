...
avg_tip_per_pas <- clean_data %>% group_by(passenger_count) %>% summarise_at(vars(tip_perc), list(mean = mean))

ggplot(data = avg_tip_per_pas, aes(y = mean,x=passenger_count, fill = passenger_count)) +
  geom_col() +  
  labs (x= "Number of Passengers", y = "Avg Tips") +
   theme(plot.title = element_text(hjust = 0.5)) +
