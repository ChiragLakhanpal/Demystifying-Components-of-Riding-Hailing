...
#list.of.packages <- c("arrow","ezids","knitr","dplyr","lubridate","ggplot2","reshape2","caret","fastDummies","dataPreparation","factoextra","AICcmodavg","modelr","glmnet","rpart","MLmetrics")
...
**Observations:** Even if the components explain 95% of the variance in the data, that may not necessarily mean that a good R^2^ or high coefficients will result. Hence to investigate this, we evaluated this; we proceed to build out Linear regression mode.
...
```{r lasso_tipfareratio1, echo=FALSE}

#reduced_train <- sample_n(train, 15000)
#reduced_test <- sample_n(test, 15000)

# Prep model parameters
x=model.matrix(tip_perc~.-tip_amount-dropoff_time-pickup_time,train)[,-1]
...
x.test <- model.matrix(tip_perc~.-tip_amount-dropoff_time-pickup_time, test)[,-1]
...
```{r coeff, results='markup',echo=FALSE}
...
```{r MSE Values, results='markup',echo=FALSE}
...
```{r Predictions, results='markup', echo=FALSE}
...
y_pred = predict(rf.fit, newdata = test['tip_perc'])
